...
class BookingRepository extends BaseRepository
{
    ...
    public function store($user, $data)
    {
        ...
        foreach($data['job_for'] as $key => $job_for) {
            ...
            if($job_for == 'certified') {
                $data['certified'] = 'yes';
            }

            if($job_for == 'certified_in_law') {
                $data['certified'] = 'law';
            }

            if($job_for == 'certified_in_helth') {
                $data['certified'] = 'health';
            }
        }    

        if(array_intersect(['normal', 'certified'], $data['job_for'])) {
            $data['certified'] = 'both';
        }

        if(array_intersect(['normal', 'certified_in_law'], $data['job_for'])) {
            $data['certified'] = 'n_law';
        }
        
        if(array_intersect(['normal', 'certified_in_helth'], $data['job_for'])) {
    ...
    public function storeJobEmail($data)
    {
        try {
            ...
            $user_email_job_id = $data['user_email_job_id'] ?? null;
            
            $job = Job::findOrFail($user_email_job_id);
    ...
    public function getPotentialJobIdsWithUserId($user_id)
    {
        ...
        /* default: for volunteers only show unpaid jobs. */
        $job_type = 'unpaid';
        ...
        /* Optimized Query */
        $userlanguage = UserLanguages::select('lang_id')
        ...
        /*
            * Job::getJobs should return Job eloquent model
            * it look like it returns job ids only
            * if it returns Job model instances we can save one query do DB in loop
            * $job = Job::find($v->id); 
        */
            ...
            if ($oneUser->user_type == '2' && $oneUser->status == '1' && $oneUser->id != $exclude_user_id) { // user is translator and he is not disabled
                ...
                foreach ($jobs as $oneJob) {
                    ...
                    if ($job->id == $oneJob->id) { // one potential job is the same with current job
                        ...
                        if ($job_for_translator == 'SpecificJob' && $job_checker != 'userCanNotAcceptJob') {

                            $this->isNeedToDelayPush($oneUser->id) 
...
//        $jobs = TeHelper::convertJobIdsInObjs($job_ids);
    ...
    public function updateJob($id, $data, $cuser)
    {
        ...
        $current_translator = 
                    $job->translatorJobRel->where('cancel_at', null)->first() 
...
//        if (in_array($data['status'], ['withdrawnbefore24', 'withdrawafter24', 'timedout'])) {
        ...
        return true;
//        }
        /*below return will not run, commenting it*/
        // return false;
    }
    ...
    public function cancelJobAjax($data, $user)
    {
        ...
        */
        ...
        if ($cuser->is('customer')) {
            ...
            $job->withdraw_at->diffInHours($job->due) >= 24
...
//        $jobs = TeHelper::convertJobIdsInObjs($job_ids);
    ...
    public function getAll(Request $request, $limit = null)
    {
        ...
        $allJobs = Job::query()
                    ->when(isset($requestdata['id']) && $requestdata['id'] != '', function ($query, $role) use (& $requestdata) {
                        // $requestdata = array_only($requestdata, ['id']);
                        return $query->where('id', $requestdata['id']);
                    })
                    ->when($consumer_type == 'RWS', function ($query, $sortByVotes) {
        ...
        $allJobs
            ->when(isset($requestdata['lang']) && $requestdata['lang'] != '', function ($query) use ($requestdata) {
            ...
            ->when(isset($requestdata['job_type']) && $requestdata['job_type'] != '', function ($query) use ($requestdata) {
            ...
            ->when(
                ...
                function ($query) use ($requestdata, & $to, & $orderBy) {
        ...
        if ($cuser && $cuser->user_type == env('SUPERADMIN_ROLE_ID')) {
            /* can be further refactored using when() */
            $allJobs = Job::query();
        ...
        $allJobs = ($allJobs == 'all') ? $allJobs->get() : $allJobs->paginate(15);
    ...
    public function alerts()
    {
        ...
        $orderBy = ['col' => 'jobs.created_at', 'dir' => 'desc'];  
        $to = null; 
        ...
        if ($cuser && $cuser->is('superadmin')) {
            $allJobs = DB::table('jobs')
                ...
                ->when(isset($requestdata['translator_email']) && $requestdata['translator_email'] != '', function ($query) use ($requestdata) {
                ...
                ->when(isset($requestdata['filter_timetype']) && $requestdata['filter_timetype'] == "created", 
                    function ($query) use ($requestdata, & $to) {
                        if (isset($requestdata['from']) && $requestdata['from'] != "") {
                ...
                ->when(isset($requestdata['filter_timetype']) && $requestdata['filter_timetype'] == "due", 
                    function ($query) use ($requestdata, & $to) {
                        ...
                        $orderBy = ['col' => 'jobs.due', 'dir' => 'desc'];   

                        return $query;
                })
                ->when(isset($requestdata['job_type']) && $requestdata['job_type'] != '',
                ...
                ->orderBy($orderBy['col'], $orderBy['desc'])
                ->paginate(15);
        }
